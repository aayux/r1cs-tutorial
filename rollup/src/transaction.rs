...
impl TransactionVar {
    ...
    pub fn validate(
        ...
        // Safely subtract amount sent from the sender's balance
        post_sender_acc_info.balance = post_sender_acc_info.balance.checked_sub(
                                            &self.amount
                                        )?;

        // Compute the new receiver balance, ensure its overflow safe.
        let mut post_recipient_acc_info = pre_recipient_acc_info.clone();
        post_recipient_acc_info.balance = post_recipient_acc_info.balance.
                                            checked_add(
                                                &self.amount
                                            )?;
